...
def find_subdirectories(base_dir, pattern_prefix="carla_nuscene_on_nuscene_0shot"):
    ...
    # Sort by rend value first, then by td value
    subdirs.sort(key=lambda x: (x[2], x[1]))
    return subdirs
...
def add_spacing_rows(df):
    """
    Add empty rows between different rend values (changed from td values)
    """
    if df.empty:
        return df
    
    # Extract rend values from row names (changed from td values)
    rend_values = []
    for idx in df.index:
        if pd.notna(idx) and idx != '':
            rend_match = re.match(r'td\d+_rend(\d+)', str(idx))
            if rend_match:
                rend_values.append(int(rend_match.group(1)))
            else:
                rend_values.append(-1)
        else:
            rend_values.append(-1)
    ...
    prev_rend = None
    for i, (idx, rend_val) in enumerate(zip(df.index, rend_values)):
        # Add spacing row if rend value changed (but not for the first row)
        if prev_rend is not None and rend_val != prev_rend and rend_val != -1:
            new_rows.append(pd.Series([np.nan] * len(df.columns), index=df.columns))
            new_indices.append('')
        
        new_rows.append(df.iloc[i])
        new_indices.append(idx)
        prev_rend = rend_val if rend_val != -1 else prev_rend
...
def create_visualization(df, rankings, output_prefix):
    ...
    # Add title
    plt.title('Score Analysis Table (Grouped by rend value)\n(Red=1st, Orange=2nd, Yellow=3rd best values)', 
...
def create_line_plots(df, output_prefix):
    """
    Create line plots for each metric with different colors for each rend value (changed from td)
    """
    ...
    # Get unique rend values and sort them (changed from td values)
    unique_rends = sorted(list(set([rend for _, _, rend in td_rend_data if rend >= 0])))
    
    # Define colors for different rend values (changed from td values)
    colors = plt.cm.Set1(np.linspace(0, 1, len(unique_rends)))
    rend_color_map = {rend: colors[i] for i, rend in enumerate(unique_rends)}
    ...
    for metric_idx, metric_col in enumerate(metric_cols):
        ...
        # Group data by rend value (changed from td value)
        rend_groups = {}
        for i, (row_name, td_val, rend_val) in enumerate(td_rend_data):
            if rend_val >= 0:
                if rend_val not in rend_groups:
                    rend_groups[rend_val] = {'x': [], 'y': [], 'labels': [], 'missing': []}
                
                rend_groups[rend_val]['x'].append(i)
                rend_groups[rend_val]['y'].append(df_clean.iloc[i][metric_col])
                rend_groups[rend_val]['labels'].append(row_name)
                ...
                rend_groups[rend_val]['missing'].append(is_missing)
        
        # Plot lines for each rend value (changed from td value)
        for rend_val in sorted(rend_groups.keys()):
            group = rend_groups[rend_val]
            x_vals = group['x']
            y_vals = group['y']
            ...
            while i < len(x_vals):
                if not missing[i]:
                    ...
                    ax.plot(seg_x, seg_y, 
                           color=rend_color_map[rend_val], 
                           linewidth=2.5, 
                           marker='o', 
                           markersize=6,
                           label=f'rend{rend_val}' if len(continuous_x) == 0 else "")
    ...
    fig.suptitle('Metric Trends by Configuration (Grouped by rend value)\n(Dotted lines indicate missing data)', 
    ...
    for metric_col in metric_cols:
            ...
            while i < len(x_vals):
                if not missing[i]:
                    ...
                    ax.plot(seg_x, seg_y, 
                           ...
                           marker='o', 
                           markersize=8,
                           label=f'rend{rend_val}' if first_line else "")
                    first_line = False
                else:
        ...
        ax.set_title(f'{metric_col} by Configuration (Grouped by rend value)', fontsize=16, fontweight='bold')
...
Example usage:
  ...
  python3 score_analysis.py ./experiments --output results
    ...
    # Add spacing rows between different rend values (changed from td values)
