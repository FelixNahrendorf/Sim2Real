...
Supports all 9 metrics: brightness, sharpness, vignetting, compression_artifacts,
...
def create_comparison_plots(csv_files, labels, output_dir):
    ...
    # Order by ALL_METRICS so output is deterministic
    ordered_metrics = [m for m in ALL_METRICS if m in common_metrics]
    extra_metrics   = sorted(common_metrics - set(ALL_METRICS))
    ordered_metrics += extra_metrics

    print(f"Found {len(ordered_metrics)} common metrics: {ordered_metrics}")
    ...
    # Combined plot (3×3 grid for up to 9 metrics, falls back gracefully)
    print("Creating combined comparison plot...")
    _plot_combined(datasets, ordered_metrics, output_path / "comparison_all_metrics.png")
...
def _plot_metric(datasets, metric, output_path):
    """Create an individual comparison plot for one metric."""
    ...
    for i, (label, hist) in enumerate(hists):
        ...
        if use_shared_axis:
            ...
            ax_main.axvline(hist['average'], color=dark_color, linestyle='--', linewidth=2.5,
                            label=f'{label} Avg: {hist["average"]:.4f}')
        else:
            ax = axes_extra[i]
    ...
    delta_parts = [f'Δ({label} vs {hists[0][0]}) = {abs(avg - avgs[0]):.4f}'
...
def _plot_combined(datasets, metrics, output_path):
    """Create a combined subplot grid for all available metrics (3 columns, dynamic rows)."""
    ...
    fig, axes = plt.subplots(nrows, ncols, figsize=(22, 6 * nrows))
    ...
    for i, metric in enumerate(metrics):
        ...
        delta_parts = [f'Δ{label}={abs(avg - avgs[0]):.4f}'
                       for (label, _), avg in zip(hists[1:], avgs[1:])]
        ...
        title_text = (f'{metric_title}\n'
                      + ' | '.join(f'{l}: {h["average"]:.4f}' for l, h in hists) + '\n'
                      + ' | '.join(delta_parts))
    ...
    plt.tight_layout()
    plt.subplots_adjust(bottom=0.08, top=0.93, hspace=0.45, wspace=0.30)
    plt.savefig(output_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Saved combined plot: {output_path}")

    # Print summary table
    print(f"\nSummary of Averages and Differences (vs {datasets[0][0]}):")
    col_w = 24
    for metric in metrics:
        ...
        metric_name = METRIC_TITLES.get(metric, metric.replace('_', ' ').title())
        avgs_str    = '  '.join(f'{label}={h["average"]:.4f}' for label, h in hists)
        deltas      = '  '.join(f'Δ{label}={abs(h["average"] - base_avg):.4f}'
                                for label, h in hists[1:])
        print(f"  {metric_name:{col_w}s}: {avgs_str}  |  {deltas}")
...
def main():
    ...
    print(f"Metrics supported: {', '.join(ALL_METRICS)}")
    print("=" * 60)
