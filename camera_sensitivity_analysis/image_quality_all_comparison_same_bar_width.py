...
def _plot_metric(datasets, metric, output_path):
    ...
    # Use a single consistent bin width across all datasets (minimum found)
    uniform_width = min(
        (hist['bin_ends'] - hist['bin_starts']).mean() for _, hist in hists
    )
    ...
    for i, (label, hist) in enumerate(hists):
        ...
        if use_shared_axis:
            ...
            ax_main.bar(hist['bin_centers'], freqs_norm,
                        width=uniform_width, alpha=0.45, color=color,
            ...
            ax.bar(hist['bin_centers'], hist['frequencies'],
                   width=uniform_width, alpha=0.55, color=color,
        ...
        for j, (label, hist) in enumerate(hists):
            ...
            if use_shared_axis:
                ...
                ax1.bar(hist['bin_centers'], freqs_norm,
                        width=uniform_width, alpha=0.40, color=color,
