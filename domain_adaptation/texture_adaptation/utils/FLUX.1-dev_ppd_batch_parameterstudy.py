...
"""
Parameter Analysis Script for FLUX.1-dev on CARLA SEED4D dataset
Tests different RADIUS and GUIDANCE_SCALE combinations
Processes spawn_point_1: 6 nuscenes + 6 selected sphere images

Run from project root:
cd /app/felix/code/NeuralRemaster/PPD-examples
PYTHONPATH=. python examples/flux/model_inference/FLUX.1-dev_ppd_batch_analysis.py
"""

# ============================================================================
# CONFIGURATION - New Parameter Study
# ============================================================================
...
BASE_OUTPUT_DIR = "/app/felix/data/neuralremaster/flux1.1/parameter_analysis_2"

# Parameter ranges for analysis
RADIUS_VALUES = [75, 80, 85, 90, 100, 120, 150, 200]  # 8 values
GUIDANCE_VALUES = [round(1.8 + x * 0.2, 1) for x in range(7)]  # 1.8, 1.9, 2.0, 2.1, 2.2, 2.3, 2.4 (7 values)

# Sphere images to process (indices 1, 10, 30, 50, 70, 80)
SPHERE_IMAGE_INDICES = [1, 10, 30, 50, 70, 80]
...
def collect_nuscenes_images():
    """Collect all 6 NuScenes RGB images"""
    if not os.path.exists(NUSCENES_DIR):
        print(f"WARNING: Directory not found: {NUSCENES_DIR}")
        return []
...
def collect_selected_sphere_images():
    """Collect specific sphere images by index (1, 10, 30, 50, 70, 80)"""
    if not os.path.exists(SPHERE_DIR):
        print(f"WARNING: Directory not found: {SPHERE_DIR}")
        return []
    
    selected_images = []
    for idx in SPHERE_IMAGE_INDICES:
        filename = f"{idx}_rgb.png"
...
def process_parameter_combination(pipe, image_list, radius, guidance_scale):
    ...
    # Create output directory with parameter naming
    output_dir = os.path.join(BASE_OUTPUT_DIR, f"spawnpoint_1_rad{radius:02d}_guid{guidance_scale:.1f}")
...
def main():
    print("\n" + "="*70)
    print("FLUX.1-dev Parameter Analysis - spawn_point_1")
    print("="*70)
    print(f"Parameter ranges:")
    print(f"  RADIUS: {RADIUS_VALUES[0]} to {RADIUS_VALUES[-1]} (step 2)")
    ...
    # Collect images
    nuscenes_images = collect_nuscenes_images()
    sphere_images = collect_selected_sphere_images()
    all_images = nuscenes_images + sphere_images
    
    print(f"\nCollected images:")
    print(f"  NuScenes: {len(nuscenes_images)}")
    print(f"  Sphere (selected): {len(sphere_images)}")
    ...
    for radius, guidance in itertools.product(RADIUS_VALUES, GUIDANCE_VALUES):
        ...
        processed, errors = process_parameter_combination(pipe, all_images, radius, guidance)
    ...
    print(f"Images per combination: {len(all_images)}")
    print(f"Total images processed: {total_processed}")
    print(f"Total errors: {total_errors}")
    print(f"\nResults saved to: {BASE_OUTPUT_DIR}")
    print(f"\nDirectory naming format: spawnpoint_1_radXX_guidX.X")
    print(f"  Example: spawnpoint_1_rad50_guid2.5")
    print("="*70 + "\n")
