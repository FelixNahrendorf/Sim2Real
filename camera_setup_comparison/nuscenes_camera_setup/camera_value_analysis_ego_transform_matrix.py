#!/usr/bin/env python3
"""
Ego Transform Matrix Analysis Tool

Analyzes transformation matrices and camera intrinsics from ego transform matrix JSON files 
and generates comprehensive visualizations and statistics for TX, TY, TZ, R11-R33, fx, fy, cx, cy.

Usage:
    python3 ego_transform_analysis.py <path_pattern> --output-dir <output_directory>
    
Example:
    python3 ego_transform_analysis.py "*_ego_transform_matrix.json" --output-dir results
    python3 ego_transform_analysis.py "/data/*_ego_transform_matrix.json" --output-dir analysis_output
"""
...
def load_ego_transform_data(file_path):
    """Load ego transform data from JSON file and extract transformation matrix components and intrinsics."""
    try:
        ...
        for frame in frames:
            ...
            tx_values.append(transform_matrix[0, 3])
            ty_values.append(transform_matrix[1, 3])
            tz_values.append(transform_matrix[2, 3])
            
            # Extract rotation matrix components (R11-R33)
            r11_values.append(transform_matrix[0, 0])
            r12_values.append(transform_matrix[0, 1])
            r13_values.append(transform_matrix[0, 2])
            r21_values.append(transform_matrix[1, 0])
            r22_values.append(transform_matrix[1, 1])
            r23_values.append(transform_matrix[1, 2])
            r31_values.append(transform_matrix[2, 0])
            r32_values.append(transform_matrix[2, 1])
            r33_values.append(transform_matrix[2, 2])
            
            # Extract camera intrinsics
            fx_values.append(frame['fl_x'])
            fy_values.append(frame['fl_y'])
        ...
        result = {
            ...
            'TZ': np.array(tz_values),
            'R11': np.array(r11_values),
            'R12': np.array(r12_values),
            'R13': np.array(r13_values),
            'R21': np.array(r21_values),
            'R22': np.array(r22_values),
            'R23': np.array(r23_values),
            'R31': np.array(r31_values),
            'R32': np.array(r32_values),
            'R33': np.array(r33_values),
            'fx': np.array(fx_values),
            'fy': np.array(fy_values),
            'cx': np.array(cx_values),
            'cy': np.array(cy_values)
        }
...
def analyze_ego_transform_files(file_pattern):
    """Analyze ego transform JSON files matching the pattern."""
    camera_data = {}
    camera_stats = {}
    ...
    for file_path in json_files:
        # Get filename without extension and remove "_ego_transform_matrix" suffix
        camera_name = Path(file_path).stem
        if camera_name.endswith('_ego_transform_matrix'):
            camera_name = camera_name[:-21]  # Remove "_ego_transform_matrix"
        
        print(f"Processing: {camera_name}")
        
        data = load_ego_transform_data(file_path)
        if data is not None:
...
def get_available_params(camera_data):
    ...
    # Define parameter order preference - translation, rotation matrix, intrinsics
...
def create_visualization(camera_data, camera_stats, output_dir):
    """Create comprehensive visualization of transformation matrix and intrinsic parameters."""
    if not camera_data:
    ...
    # Calculate number of subplots needed (4 columns for better layout with 16 parameters)
    n_params = len(params)
    n_cols = 4
    ...
    fig.suptitle('Ego Transform Matrix Parameters Distribution', fontsize=16, fontweight='bold')
    ...
    # Save the plot
    png_path = output_dir / 'ego_transform_parameters_distribution.png'
    svg_path = output_dir / 'ego_transform_parameters_distribution.svg'
...
def create_individual_camera_plots(camera_data, camera_stats, output_dir):
    ...
    # Calculate number of subplots needed (4 columns for better layout)
    n_params = len(params)
    n_cols = 4
    n_rows = (n_params + n_cols - 1) // n_cols  # Ceiling division
    ...
    for camera_name, data in camera_data.items():
        ...
        fig, axes = plt.subplots(n_rows, n_cols, figsize=(24, 6 * n_rows))  # Increased width for 4 columns
        fig.suptitle(f'{camera_name} - Ego Transform Matrix Parameters (Original Values)', 
        ...
        for j, param in enumerate(params):
            ...
            if param in data:
                ...
                # Detect std dev outliers on original values (more than 1.5 std dev from mean)
                ...
                stats_text = (f'Mean: {param_stats["mean"]:.6f}\n'
                            f'Std: {param_stats["std"]:.6f}\n'
                            f'Min: {param_stats["min"]:.6f}\n'
                            f'Max: {param_stats["max"]:.6f}\n'
                ...
                ax.axvline(param_stats['mean'], color='darkblue', linestyle='--', linewidth=2, alpha=0.8, 
                          label=f'Mean: {param_stats["mean"]:.6f}')
        ...
        png_path = output_dir / f'{camera_name}_ego_transform_distributions.png'
        svg_path = output_dir / f'{camera_name}_ego_transform_distributions.svg'
...
def create_box_plots(camera_data, camera_stats, output_dir):
    ...
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(20, 5 * n_rows))
    fig.suptitle('Ego Transform Matrix Parameters - Box Plots Comparison', fontsize=14, fontweight='bold')
    ...
    for i, param in enumerate(params):
        ...
        if data_for_boxplot:  # Only create plot if there's data
            ...
            for j, camera_name in enumerate([cam for cam in available_cameras if param in camera_data[cam]]):
                ...
                stats_text_lines.append(f'{camera_short}: μ={stats["mean"]:.4f}, σ={stats["std"]:.4f}')
    ...
    # Save the plot
    png_path = output_dir / 'ego_transform_parameters_boxplots.png'
    svg_path = output_dir / 'ego_transform_parameters_boxplots.svg'
...
def create_statistics_table(camera_stats, output_dir):
    ...
    # Create output file
    stats_file = output_dir / 'ego_transform_parameters_statistics.txt'
    
    with open(stats_file, 'w') as f:
        # Write to file and print to console
        header = "="*100 + "\n"
        title = "EGO TRANSFORM MATRIX PARAMETERS STATISTICS\n"
        header2 = "="*100 + "\n"
        ...
        for param in params:
            ...
            separator = "-" * 80 + "\n"
            table_header = f"{'Camera':<20} {'Mean':<15} {'Std Dev':<15} {'Min':<15} {'Max':<15} {'Median':<15}\n"
            ...
            for camera_name, stats in camera_stats.items():
                if param in stats:
                    ...
                    row = (f"{camera_name:<20} {param_stats['mean']:<15.8f} {param_stats['std']:<15.8f} "
                          f"{param_stats['min']:<15.8f} {param_stats['max']:<15.8f} {param_stats['median']:<15.8f}\n")
                    f.write(row)
                    print(row.strip())
...
def save_statistics_csv(camera_stats, output_dir):
    ...
    csv_path = output_dir / 'ego_transform_parameters_statistics.csv'
    df.to_csv(csv_path, index=False)
    print(f"✓ Statistics saved to '{csv_path}'")

def main():
    parser = argparse.ArgumentParser(
        description='Analyze ego transform matrix parameters from JSON files',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 ego_transform_analysis.py "*_ego_transform_matrix.json" --output-dir results
  python3 ego_transform_analysis.py "/path/to/*_ego_transform_matrix.json" --output-dir analysis
  python3 ego_transform_analysis.py "CAM_*_ego_transform_matrix.json" --output-dir output
        """
    )
    
    parser.add_argument(
        'file_pattern',
        help='File pattern to match ego transform matrix JSON files (e.g., "*_ego_transform_matrix.json")'
    )
    ...
    # Analyze ego transform files
    print(f"Searching for files matching: {args.file_pattern}")
    camera_data, camera_stats = analyze_ego_transform_files(args.file_pattern)
    
    if not camera_data or not camera_stats:
        print("No valid ego transform data found. Please check your file pattern and formats.")
        sys.exit(1)
    
    print(f"\n✓ Successfully loaded {len(camera_data)} ego transform files")
