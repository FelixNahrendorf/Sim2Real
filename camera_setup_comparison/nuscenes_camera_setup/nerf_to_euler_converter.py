...
def adjust_to_target(value, target, tolerance=0.1):
    """
    Adjusts value to be within ±10% of target (default).
    If not within range, add/subtract 0.5π until closest to target.
    """
    ...
    # Otherwise adjust by ±0.5π until closest
    best_val = value
    best_diff = abs(value - target)

    for k in range(-10, 11):  # shift up to ±10 * (0.5π) just in case
...
def process_nerf_data(nerf_data: Dict) -> Dict[str, List[float]]:
    """
    ...
    for frame in frames:
        ...
        try:
            ...
            yaws.append(adjust_to_target(yaw, target_yaws[target_index]))
...
def process_single_nerf_file(file_path: str, output_dir: str) -> bool:
    """
    ...
    # Process the data
    processed_data = process_nerf_data(nerf_data)
...
def process_nerf_files(input_pattern: str, output_dir: str = ".") -> None:
    """
    Process multiple NeRF JSON files matching the input pattern.
    
    Args:
        input_pattern: File pattern to match (e.g., "*.json" or specific file)
        output_dir: Output directory for processed files
    """
    ...
    for file_path in files:
        if process_single_nerf_file(file_path, output_dir):
            successful += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process NeRF-style JSON files with 4x4 transformation matrices and extract coordinates, roll, pitch, yaw, and FOV",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 nerf_transform_converter.py transforms_ego.json
  python3 nerf_transform_converter.py "transforms_*.json" --output-dir ./processed
  python3 nerf_transform_converter.py transforms_ego.json --output-dir ./output
        """
    )
    
    parser.add_argument("input_files", nargs="+", 
                       help="Input NeRF JSON file(s) or pattern (e.g., 'transforms_*.json')")
    ...
    print("="*60)
    print("NeRF Transform Matrix to Camera Pose Converter")
    print("="*60)
    ...
    for input_pattern in args.input_files:
        process_nerf_files(input_pattern, args.output_dir)
